...
export async function ThreeBodyAnimation(maindiv: string, configdiv: string, control_div: string) {
  ...
  document.addEventListener('click', function (event) {
    ...
    if (intersect_id == -1) return;
    if (spheres_highlight[intersect_id] == 0) {
      spheres[intersect_id].scale.set(2, 2, 2);
      spheres_highlight[intersect_id] = 1;
      let anim = addVelocityGraph(
        velocityEnvironment.scene, 
        velocityEnvironment.controls, 
        velocityEnvironment.renderer, 
        velocityEnvironment.camera, 
        intersect_id
      );
      ...
      if (currentIndex >= data_length) {
        ...
        for (let i = 0; i < velocityGraphAnimates.length; i++) {
          if (velocityGraphAnimates[i]) {
            ...
            velocityGraphAnimates[i] = addVelocityGraph(
              velocityEnvironment.scene, 
              velocityEnvironment.controls, 
              velocityEnvironment.renderer, 
              velocityEnvironment.camera, 
...
function removeVelocityGraph(scene, label: number) {
  scene.remove(scene.getObjectByName(`sphere${label}`));
  scene.remove(scene.getObjectByName(`pathLine${label}`));
}

function addVelocityGraph(scene, controls, renderer, camera, label: number) {
  let coord = 0;
...
function setupVelocities(graph_div: string, control_div: string) {
  const defaultAspectRatio = 16/9;
